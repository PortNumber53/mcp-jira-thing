...
function getSessionSecret(): string {
  ...
  if (secret) {
    if (!env.SESSION_SECRET && !emittedSessionSecretWarning) {
      console.warn("[oauth] COOKIE_ENCRYPTION_KEY is deprecated. Please migrate this Worker to SESSION_SECRET.");
...
app.get("/authorize", async (c) => {
  ...
  if (await clientIdAlreadyApproved(c.req.raw, oauthReqInfo.clientId, getSessionSecret())) {
