...
- **[goal]** Refine logging in MCP worker for level-based control
- **[done]**
  - Added `logMessage` function with `LOG_LEVEL` checks in `src/index.ts`
  - Updated all log statements to use the new function with appropriate levels (debug, info, warn, error)
- **[next]**
  - Test logging with different LOG_LEVEL settings
  - Monitor for any runtime errors or further refinements
